...
function isInsideCircle(circle, point) {
  const p1 = point.x - circle.center.x;
  const p2 = point.y - circle.center.y;
  const square = Math.sqrt(p1 ** 2 + p2) ** 2;

  return square < circle.radius;
}
...
function reverseInteger(num) {
  ...
  const numRev = numStr.split('').reverse();
  return numRev.join('');
}
